...
// https://budgeting.thenest.com/cusip-number-treasury-bond-25561.html
// 2Y, 3Y, 5Y, 7Y, 10Y, 20Y, and 30Y
string CUSIPS[] = { "9129284J6", "9129284J9", "9129284M5", "9129286P7", "9129286Q5", "9129289Q2", "9129289Q3" };

void maps_initial()
{
	bond_map[CUSIPS[0]] = Bond(CUSIPS[0], CUSIP, "T", 0.02, date(2025, 12, 24));
	bond_map[CUSIPS[1]] = Bond(CUSIPS[1], CUSIP, "T", 0.03, date(2026, 12, 24));
	bond_map[CUSIPS[2]] = Bond(CUSIPS[2], CUSIP, "T", 0.05, date(2028, 12, 24));
	bond_map[CUSIPS[3]] = Bond(CUSIPS[3], CUSIP, "T", 0.07, date(2030, 12, 24));
	bond_map[CUSIPS[4]] = Bond(CUSIPS[4], CUSIP, "T", 0.10, date(2033, 12, 24));
	bond_map[CUSIPS[5]] = Bond(CUSIPS[5], CUSIP, "T", 0.20, date(2043, 12, 24));
	bond_map[CUSIPS[6]] = Bond(CUSIPS[6], CUSIP, "T", 0.30, date(2053, 12, 24));

	Position<Bond> position0(Bond(CUSIPS[0], CUSIP, "T", 0.02, date(2025, 12, 24)));
	Position<Bond> position1(Bond(CUSIPS[1], CUSIP, "T", 0.03, date(2026, 12, 24)));
	Position<Bond> position2(Bond(CUSIPS[2], CUSIP, "T", 0.05, date(2028, 12, 24)));
	Position<Bond> position3(Bond(CUSIPS[3], CUSIP, "T", 0.07, date(2030, 12, 24)));
	Position<Bond> position4(Bond(CUSIPS[4], CUSIP, "T", 0.10, date(2033, 12, 24)));
	Position<Bond> position5(Bond(CUSIPS[5], CUSIP, "T", 0.20, date(2043, 12, 24)));
	Position<Bond> position6(Bond(CUSIPS[6], CUSIP, "T", 0.30, date(2053, 12, 24)));

	position0.position_begin();
	position1.position_begin();
	position2.position_begin();
	position3.position_begin();
	position4.position_begin();
	position5.position_begin();
	position6.position_begin();

	bond_position_map[CUSIPS[0]] = position0;
	bond_position_map[CUSIPS[1]] = position1;
	bond_position_map[CUSIPS[2]] = position2; 
	bond_position_map[CUSIPS[3]] = position3;
	bond_position_map[CUSIPS[4]] = position4;
	bond_position_map[CUSIPS[5]] = position5;
	bond_position_map[CUSIPS[6]] = position6;
